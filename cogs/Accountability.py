...
class Accountability(commands.Cog):
    def __init__(self, client, accountability_channel_id):
        ...
        with open("cogs/open_accountability_invitations.json", "r") as f:
            ...
            # json stores keys as strings no matter what, when our datatype for the keys is really int.
    ...
    async def accountability(self, ctx, *args: str):
        if ctx.channel.id != self.accountability_channel_id:
            print(
                f"{ctx.author} tried to use an accountability command outside the proper channel!"
            )
            return
    ...
    async def start(self, ctx, *args):
        ...
        if stake_level not in ["low", "medium", "high"]:
            await ctx.send(
                f"{ctx.author}, please try your command again. The stake options are low, medium, or high."
            )
        elif invited_member_obj == ctx.author:
            await ctx.send(
                f"{ctx.author}, you cannot invite yourself to an Accountability Partnership!"
            )
        elif (
            int(ctx.author.id) in self.open_invitations
            and self.open_invitations[int(ctx.author.id)][0] == invited_member_id
        ):
            await ctx.send(
                f"{ctx.author}, you have already invited {invited_member_obj} to an Accountability Partnership."
            )
        elif invited_member_id in self.open_invitations and self.open_invitations[
            int(invited_member_id)
        ][0] == int(ctx.author.id):
            ...
            if self.open_invitations[int(invited_member_id)][1] == stake_level:
                await ctx.send(
                    f"<@{invited_member_id}> and <@{ctx.author.id}>, you have begun an Accountability Partnership for {stake_level} stakes!"
                )
                del self.open_invitations[invited_member_id]
                self.save_open_invitations()
                self.start_accountability_partnership(
                    invited_member_id, ctx.author.id, stake_level
                )
            else:
                await ctx.send(
                    f"{ctx.author}, you must start the Partnership for the same stakes as your partner. Decline their invitation and try starting again to change the stakes."
                )
        else:
            await ctx.send(
                f"{ctx.author} has invited {invited_member_obj} to participate in an Accountability Partnership!"
            )
            self.open_invitations[int(ctx.author.id)] = [
                invited_member_obj.id,
                stake_level,
            ]
            self.save_open_invitations()
            await invited_member_obj.send(
                f"{invited_member_obj}, {ctx.author} has invited you to an Accountability Partnership!\nType `!accountability start {ctx.author}` in the accountability channel to accept their invitation!\nType `!accountability decline {ctx.author}` if you choose to decline."
            )
    ...
    async def decline(self, ctx, *args):
        ...
        if ctx.author.id not in all_invited_members or (
            ctx.author.id in all_invited_members
            and self.open_invitations[original_member.id][0] != ctx.author.id
        ):
            await ctx.send(
                "Sorry, you do not have an accountability invitation from this person."
            )
        elif self.open_invitations[original_member.id][0] == ctx.author.id:
            del self.open_invitations[original_member.id]
            await ctx.send(
                f"{ctx.author}, you have declined the accountability invitation from {original_member}."
            )
            self.save_open_invitations()
    ...
    async def log(self, ctx, *args):
        ...
        if ap is None:
            await ctx.send(
                "Couldn't find your Accountability Partnership! Are you sure you're in an active one?"
            )
        elif args[1] == "today":
    ...
    def start_accountability_partnership(
        self, original_member, second_member, stake_level
    ):
        AccountabilityPartnership(
            original_member,
            second_member,
            stake_level=stake_level,
            started_by=original_member,
        )
        AccountabilityPartnership(
            second_member,
            original_member,
    ...
    async def help(self, ctx, *args):
        ...
        help_embed = discord.Embed(
            title="Accountability Partnership Commands",
            description="These commands can only be used inside the accountability channel!",
            color=discord.Color.green(),
        )
        help_embed.add_field(
            name="help",
            value="What you are seeing now. Shows info for accountability commands.",
            inline=False,
        )
        help_embed.add_field(
            name="start (user)",
            value="Invite someone to participate in an Accountability Partnership by typing their username after this command.",
            inline=False,
        )
        help_embed.add_field(
            name="decline",
            value="Decline someone's Accountability Partnership invitation.",
            inline=False,
        )
        help_embed.add_field(
            name="log today/yesterday",
            value="Log today or yesterday for your Accountability Partnership.",
            inline=False,
        )
        help_embed.add_field(
            name="info",
            value="Get information about your current Accountability Partnership.",
            inline=False,
        )
        help_embed.add_field(
            name="end",
            value="Ends your current Accountability Partnership if you are in one.",
            inline=False,
        )
    ...
    async def info(self, ctx, *args):
        ...
        if ap is None:
            await ctx.send(
                f"{ctx.author.mention}, you are not in an active Accountability Partnership!"
            )
        else:
            ap_embed = discord.Embed(
                title="Accountability Partnership",
                description=f"{ctx.author.mention} and {(await ctx.guild.fetch_member(ap.other_member)).mention}",
                color=discord.Color.blue(),
            )
            ds_tt = date.fromisoformat(ap.date_started).timetuple()
            ldl_tt = (
                date.fromisoformat(ap.last_date_logged).timetuple()
                if ap.last_date_logged
                else [None] * 3
            )
            ldc_tt = (
                date.fromisoformat(ap.last_date_completed).timetuple()
                if ap.last_date_completed
                else [None] * 3
            )
            ap_embed.add_field(name="Status", value="Paused" if ap.paused else "Active")
            ap_embed.add_field(
                name="Date Started:", value=f"{ds_tt[1]}/{ds_tt[2]}/{ds_tt[0]}"
            )
            if ap.date_resumed is not None:
                dr_tt = date.fromisoformat(ap.date_resumed).timetuple()
                ap_embed.add_field(
                    name="Date Resumed:", value=f"{dr_tt[1]}/{dr_tt[2]}/{dr_tt[0]}"
                )
            ap_embed.add_field(
                name="Your Last Day Logged:",
                value=f"{ldl_tt[1]}/{ldl_tt[2]}/{ldl_tt[0]}",
            )
            ap_embed.add_field(
                name="Last Day Completed by Both Partners:",
                value=f"{ldc_tt[1]}/{ldc_tt[2]}/{ldc_tt[0]}",
            )
    ...
    async def log_today(self, ctx, ap: AccountabilityPartnership, *args):
        ...
        log_embed = discord.Embed(
            title="Accountability Partnership",
            description=f"<@{ap.primary_member}> and <@{ap.other_member}>",
        )
        if status == "paused":
            ...
            log_embed.add_field(
                ...
                value="Your Partnership is Currently Paused! Use !a resume to resume your partnership.",
                inline=False,
            )
            ...
            log_embed.color = discord.Color.red()
            log_embed.add_field(
                name="Log Status:", value="Already Logged", inline=False
            )
            log_embed.add_field(
                name="Days Logged by You:", value=ap.log_streak, inline=False
            )
            ...
            log_embed.add_field(name="Log Status:", value="Successful", inline=False)
            log_embed.add_field(
                name="Days Logged by You:", value=ap.log_streak, inline=False
            )
            print(f"Log streak was {ap.log_streak}")
            log_embed.add_field(
                name="Days Completed by Both Partners:",
                value=ap.completion_streak,
                inline=False,
            )
            ...
            if ap.added_points:
                completed_embed = discord.Embed(
                    title=f"Accountability Day {ap.completion_streak} Completed!",
                    description=f"<@{ap.primary_member}> and <@{ap.other_member}>",
                    color=discord.Color.gold(),
                )
                completed_embed.add_field(
                    name="Points Gained by Both Partners Today:", value=2
                )
                completed_embed.add_field(
                    name=f"{ctx.author} Total Points:",
                    value=f"{get_points_by_id(ctx.author.id)}",
                )
                completed_embed.add_field(
                    name=f"{await ctx.guild.fetch_member(ap.other_member)} Total Points:",
                    value=f"{get_points_by_id(ap.other_member)}",
                )
            other_ap = ap.get_other_member_ap()
            if other_ap is None:
                await ctx.send("There's been a glitch. Please contact Elias.")
            elif ap.date_obj_from_str(ap.last_date_logged) > ap.date_obj_from_str(
                ...
                await other_member_obj.send(
                    f"{other_member_obj}, your Accountability Partner has logged for today!\nPlease log today to complete the day and extend your streak!"
                )
        else:
            log_embed.color = discord.Color.red()
            log_embed.add_field(
                name=f"{ctx.author} Log Status:",
                value="You forgot to log yesterday!",
                inline=False,
            )
            log_embed.add_field(
                name="Days Logged by You:", value=ap.log_streak, inline=False
            ...
            log_embed.add_field(name="Points Gained:", value=0, inline=False)
    ...
    async def log_yesterday(self, ctx, ap: AccountabilityPartnership, *args):
        ...
        if status == "paused":
            ...
            if ap.added_points:
                ...
                completed_embed.add_field(
                    name="Points Gained by Both Partners Yesterday:", value=2
                )
                completed_embed.add_field(
                    name=f"{ctx.author} Total Points:",
                    value=f"{get_points_by_id(ctx.author.id)}",
            ...
            if other_ap is None:
                ...
                # send reminder
                print(
                    f"Other partner last_date_logged is {other_ap.get_last_date_logged()}"
                )
                other_member_obj: discord.User = ctx.guild.get_member(ap.other_member)
                await other_member_obj.send(
                    f"{other_member_obj}, your Accountability Partner has logged for yesterday!\nPlease log yesterday to complete the day and extend your streak!"
                )
        await ctx.send(embed=log_embed)
        try:
            await ctx.send(embed=completed_embed)
    ...
    async def end_partnership(self, ctx, *args):
        ...
        if ap is None:
            ...
            await ctx.send(
                f"<@{ap.primary_member}> and <@{ap.other_member}>, your Accountability Partnership has ended."
            )
    ...
    async def pause(self, ctx, *args):
        ...
        if ap:
            ...
            if pause_status:
                await ctx.send(
                    f"{ctx.author}, your Accountability Partnership has been paused."
                )
                other_member_obj: discord.User = ctx.guild.get_member(ap.other_member)
                await other_member_obj.send(
                    f"{ctx.author} has paused your Accountability Partnership."
                )
            else:
                await ctx.send(
                    f"{ctx.author}, your Accountability Partnership was already paused."
                )
    ...
    async def resume(self, ctx, *args):
        ...
        if ap:
            ...
            if resume_status:
                await ctx.send(
                    f"{ctx.author}, your Accountability Partnership has been resumed!"
                )
                other_member_obj: discord.User = ctx.guild.get_member(ap.other_member)
                await other_member_obj.send(
                    f"{ctx.author} has resumed your Accountability Partnership!"
                )
            else:
                await ctx.send(
                    f"{ctx.author}, your Accountability Partnership was not paused."
                )
...
async def get_member_obj_from_arg(ctx, arg):
    ...
    if invited_member_obj is None and arg.startswith("<@") and arg.endswith(">"):
