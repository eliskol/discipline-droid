...
class Accountability(commands.Cog):
    ...
    async def start(self, ctx, *args):
        ...
        if stake_level not in ["low", "medium", "high"]:
            ...
            self.open_invitations[int(ctx.author.id)] = [
                invited_member_obj.id, stake_level]
            self.save_open_invitations()
    ...
    async def decline(self, ctx, *args):
        ...
        all_invited_members = [arr[0]
                               for arr in self.open_invitations.values()]
    ...
    def start_accountability_partnership(self, original_member, second_member, stake_level):
        AccountabilityPartnership(
            original_member, second_member, stake_level=stake_level, started_by=original_member)
        AccountabilityPartnership(
            second_member, original_member, stake_level=stake_level, started_by=original_member)

    async def help(self, ctx, *args):
        print(f"help command invoked by {ctx.author}")
        help_embed = discord.Embed(title="Accountability Partnership Commands",
                                   description="These commands can only be used inside the accountability channel!", color=discord.Color.green())
        help_embed.add_field(
            name="help", value="What you are seeing now. Shows info for accountability commands.", inline=False)
        help_embed.add_field(
            name="start (user)", value="Invite someone to participate in an Accountability Partnership by typing their username after this command.", inline=False)
        help_embed.add_field(
            name="decline", value="Decline someone's Accountability Partnership invitation.", inline=False)
        help_embed.add_field(name="log today/yesterday",
                             value="Log today or yesterday for your Accountability Partnership.", inline=False)
        help_embed.add_field(
            name="info", value="Get information about your current Accountability Partnership.", inline=False)
        help_embed.add_field(
            name="end", value="Ends your current Accountability Partnership if you are in one.", inline=False)
    ...
    async def info(self, ctx, *args):
        ...
        if ap is None:
            ...
            ) if ap.last_date_logged else [None] * 3
            ldc_tt = date.fromisoformat(ap.last_date_completed).timetuple(
            ) if ap.last_date_completed else [None] * 3
            ap_embed.add_field(
                name="Status", value="Paused" if ap.paused else "Active")
            ap_embed.add_field(name=f"Date Started:",
                               value=f"{ds_tt[1]}/{ds_tt[2]}/{ds_tt[0]}")
            if ap.date_resumed is not None:
                dr_tt = date.fromisoformat(ap.date_resumed).timetuple()
                ap_embed.add_field(name="Date Resumed:",
                                   value=f"{dr_tt[1]}/{dr_tt[2]}/{dr_tt[0]}")
            ap_embed.add_field(name="Your Last Day Logged:",
                               value=f"{ldl_tt[1]}/{ldl_tt[2]}/{ldl_tt[0]}")
            ap_embed.add_field(name="Last Day Completed by Both Partners:",
    ...
    async def log_today(self, ctx, ap: AccountabilityPartnership, *args):
        ...
        log_embed = discord.Embed(title="Accountability Partnership",
                                  description=f"<@{ap.primary_member}> and <@{ap.other_member}>")
        if status == "paused":
            print("Building paused embed now.")
            log_embed.color = discord.Color.red()
            log_embed.add_field(name=f"Partnership Status:",
                                value="Your Partnership is Currently Paused! Use !a resume to resume your partnership.", inline=False)
        elif status == "already logged":
            print("Building already logged embed now.")
            # await ctx.send("You have already logged for today!")
            log_embed.color = discord.Color.red()
            log_embed.add_field(name=f"Log Status:",
                                value="Already Logged", inline=False)
            log_embed.add_field(name=f"Days Logged by You:",
                                value=ap.log_streak, inline=False)
            ...
            log_embed.add_field(name=f"Days Completed by Both Partners:",
                                value=ap.completion_streak, inline=False)
            ...
            if ap.added_points:
                completed_embed = discord.Embed(title=f"Accountability Day {ap.completion_streak} Completed!",
                                                description=f"<@{ap.primary_member}> and <@{ap.other_member}>", color=discord.Color.gold())
                completed_embed.add_field(
                    name=f"Points Gained by Both Partners Today:", value=2)
                completed_embed.add_field(
                    name=f"{ctx.author} Total Points:", value=f"{get_points_by_id(ctx.author.id)}")
            ...
            if other_ap is None:
                ...
                # send reminder
                print(
                    f"Other partner last_date_logged is {other_ap.last_date_logged}")
                other_member_obj: discord.User = ctx.guild.get_member(
                    ap.other_member)
            ...
            log_embed.color = discord.Color.red()
            log_embed.add_field(
                name=f"{ctx.author} Log Status:", value="You forgot to log yesterday!", inline=False)
            log_embed.add_field(name=f"Days Logged by You:",
                                value=ap.log_streak, inline=False)
            log_embed.add_field(name=f"Days Completed by Both Partners:",
                                value=ap.completion_streak, inline=False)
        ...
        if status == "paused":
            ...
            if ap.added_points:
                completed_embed = discord.Embed(title=f"Accountability Day {ap.completion_streak} Completed!",
                                                description=f"<@{ap.primary_member}> and <@{ap.other_member}>", color=discord.Color.gold())
                completed_embed.add_field(
                    name=f"Points Gained by Both Partners Yesterday:", value=2)
                completed_embed.add_field(
                    name=f"{ctx.author} Total Points:", value=f"{get_points_by_id(ctx.author.id)}")
                completed_embed.add_field(name=f"{await ctx.guild.fetch_member(ap.other_member)} Total Points:", value=f"{get_points_by_id(ap.other_member)}")
