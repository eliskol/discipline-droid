...
class Habits(commands.Cog):
    def __init__(self, client):
        self.client = client
        self.discipline_to_leaderboard_json_title = {
            "makebed": "Makebed",
            "alarm": "Alarm",
            "sunriser": "Early",
            "meditation": "Meditate",
    ...
    def get_longest_current_streak_for_discipline(self, discipline_name) -> tuple[int]:
        ...
        discipline_record = pd.read_csv(f"cogs/Habits Record/{discipline_name}.csv")
    ...
    async def remove_extra_roles(self, ctx):
        ...
        discipline_roles = [
            ...
            if role != officer_role and role != admin_role
        ][1:-1]
        if len(discipline_roles) != 0:
            await ctx.author.remove_roles(*discipline_roles)

    async def input_discipline(
        self, discipline, ctx: commands.Context, yesterday=False
    ):
        ...
        col_index_of_date_to_input = (
            dates_in_file_as_strings.index(today_iso_date) - yesterday
        )
        ...
        if discipline_record.iloc[row_index_for_user, col_index_of_date_to_input] == 1:
            eco_embed = discord.Embed(
                title=self.disciplines[discipline]["alr_done"]["title"],
                description=f"{self.disciplines[discipline]['alr_done']['description']} {ctx.author.mention}",
        ...
        eco_embed = discord.Embed(
            title=self.disciplines[discipline]["just_done"]["title"],
            description=f"{self.disciplines[discipline]['just_done']['description']} {ctx.author.mention}",
            color=discord.Color.green(),
        )
        eco_embed.add_field(
            name="Points Earned:", value=f"{points_discipline_is_worth}", inline=False
        )
        eco_embed.add_field(
            name="Total Growth Points:",
            value=f"{user_point_totals[str(ctx.author.id)]['Growth Points']}",
            inline=False,
        )
        eco_embed.add_field(
            name=f"{self.disciplines[discipline]['long_name']} Streak:",
        ...
        if gp >= next_role_points:
            ...
            await channelm.send(
                f"Congratulations {ctx.author.mention}! You Have Exemplified Discipline and Have Leveled Up to {next_role_name}"
            )
    ...
    async def update_leaderboard_message(self):
        ...
        await self.client.leaderboard_message.edit(embed=new_embed)
    ...
    async def habits(self, context: commands.Context, *args: str):
        ...
        if arg == "allmonth":
            if self.generate_discipline_record_for_member(context.author):
                ...
                await context.send(
                    f"{context.author}, you don't have any discipline records for this month!"
                )
            return
        ...
        detected_discipline = [
            ...
            if (arg in self.disciplines[discipline]["aliases"] or arg == discipline)
        ]
        ...
        detected_yesterday_discipline = [
            discipline
            for discipline in self.disciplines
            if (
                arg in self.disciplines[discipline]["yesterday_aliases"]
                or arg == "yesterday" + discipline
            )
        ]
        if detected_yesterday_discipline != []:
            assert len(detected_yesterday_discipline) == 1
            print(f"detected {detected_yesterday_discipline[0]} from {context.author}")
            await self.input_discipline(
                detected_yesterday_discipline[0], context, yesterday=True
            )
            return

        elif args[0].endswith("week"):
            detected_week_command = [
                discipline
                for discipline in self.disciplines
                if (
                    arg.removesuffix("week") in self.disciplines[discipline]["aliases"]
                    or arg.removesuffix("week") == discipline
                )
            ]
            if detected_week_command != []:
                assert len(detected_week_command) == 1
                print(f"detected {detected_week_command[0]}week from {context.author}")
                await self.disciplineweek(detected_week_command[0], context)
                return

        elif args[0].endswith("month"):
            detected_month_command = [
                discipline
                for discipline in self.disciplines
                if (
                    arg.removesuffix("month") in self.disciplines[discipline]["aliases"]
                    or arg.removesuffix("month") == discipline
                )
            ]
    ...
    async def alllastmonth(self, ctx):
        if self.generate_discipline_record_for_member(ctx.author, True):
            ...
            await ctx.send(
                f"{ctx.author}, you don't have any discipline records for last month!"
            )
    ...
    async def ranks(self, ctx):
        ...
        output = t2a(
            header=["Rank", "Points"],
            body=[list(row) for row in rolesf.to_numpy()],
    ...
    async def disciplineweek(self, discipline, ctx):
        ...
        if str(ctx.author.id) not in names:
            ...
            record = pd.concat([record, new_row_series_transpose], ignore_index=True)
        ...
        output = t2a(
            header=[
                f"This Week's {self.disciplines[discipline]['long_name']}",
                Merge.LEFT,
            ],
    ...
    async def disciplinemonth(self, discipline, ctx):
        ...
        for i in list(range(len(month_record_formatted))):
            fullm.append(month_day_numbers_so_far[i])
            fullm.append(month_record_formatted[i])

        header = [f"This Month's {self.disciplines[discipline]['long_name']}"]
        ...
        output = t2a(header=header, body=fullm, style=PresetStyle.double_thin_box)
        await ctx.send(f"```\n{output}\n```")
    ...
    async def today(self, ctx: commands.Context):
        ...
        for discipline in self.disciplines:
            today_result = (
                pd.read_csv(f"cogs/Habits Record/{discipline}.csv")
                .query(f'Member == "{user_id}"')
                .filter(like=today_string, axis=1)
                .iloc[0, 0]
            )
            today_results[discipline] = today_result
        disciplines_row_1 = [
            discipline.capitalize() for discipline in list(self.disciplines.keys())[::2]
        ]
        disciplines_row_2 = [
        ...
        for i in range(len(disciplines_row_1)):
            row_1_indicator = (
                "\u2713" if today_results[disciplines_row_1[i].lower()] == 1 else " "
            )
            row_2_indicator = (
                "\u2713" if today_results[disciplines_row_2[i].lower()] == 1 else " "
            )
            table_body.append(
                [
                    disciplines_row_1[i],
    ...
    async def show(self, ctx, *usernames):
        ...
        members = [
            self.get_member_obj_from_username(ctx, username) for username in usernames
        ]
        ...
        for member in members:
            if self.generate_discipline_record_for_member(member):
                ...
                await ctx.send(
                    f"{ctx.author}, you do not have any discipline records for last month!"
                )
    ...
    def generate_discipline_record_for_member(self, member, last_mo=False):
        ...
        for discipline in self.disciplines:
            ...
            dataframe = (
                pd.read_csv(f"cogs/Habits Record/{discipline}.csv")
                .query(f'Member == "{user_id}"')
                .filter(like=date_prefix, axis=1)
            )
        ...
        for col in range(cols):
            for row in range(rows):
                if int(discipline_dataframe.iloc[row, col]) == 1:
                    ax.text(
                        x=col, y=row, s="\u2713", va="center", ha="center", fontsize=24
                    )

        for col in range(cols):
            ax.text(col, 9.75, col + 1, weight="bold", ha="center", fontsize=20)
            ax.plot(
                [col - 0.5, col - 0.5], [-0.5, rows + 0.5], ls="solid", lw="1", c="grey"
            )
            if col % 10 == 0:
                ax.plot(
                    [col - 0.5, col - 0.5],
        ...
        ax.plot([bb, cols - 0.5], [9.5, 9.5], ls="solid", lw="2.4", c="black")
        ax.plot([bb, cols - 0.5], [10.5, 10.5], ls="solid", lw="3", c="black")
        ax.plot([bb, cols - 0.5], [-0.5, -0.5], ls="solid", lw="3", c="black")
        ax.plot([bb, bb], [-0.5, rows + 0.5], ls="solid", lw="3", c="black")
        ax.plot(
            [cols - 0.5, cols - 0.5], [-0.5, rows + 0.5], ls="solid", lw="3", c="black"
        )
